...
public class ReservacionDAO {
	...
	public ArrayList<ObtenerListaReservacionesResponse.Reservacion> getListaReservaciones(){
		...
		this.database = new Conexion();
		try {
			ResultSet rs = this.database.connect().createStatement().executeQuery("SELECT * FROM Reservacion");
			while(rs.next()) {
				ObtenerListaReservacionesResponse.Reservacion reservacion = 
						new ObtenerListaReservacionesResponse.Reservacion(rs.getInt("numeroReservacion"), rs.getString("usuarioRes"), 
								rs.getInt("numHabR"),rs.getDate("fechaini").toString(),rs.getDate("fechafin").toString());
